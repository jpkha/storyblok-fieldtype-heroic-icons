...
export default {
  ...
  computed: {
    filteredIcons() {
      ...
      iconsName.forEach((icon) => {
        result.push({icon: icon, type: 'heroic-icon-outline'})
        result.push({icon: icon, type: 'heroic-icon-solid'})
      })
  ...
  methods: {
    getSvg(icon) {
      if (icon.type === 'heroic-icon-outline' && this.heroIconsOutline) {
      ...
      } else if (icon.type === 'heroic-icon-solid' && this.heroIconsSolid) {
